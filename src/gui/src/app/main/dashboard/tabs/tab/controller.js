...
.controller('TabController', function($scope, $http, $modal, $filter, ChartService, $timeout) {
	...
	$scope.startUp = function(ccd) {
		if (!isEmpty(ccd)) {
			$scope.patient = ccd.recordTarget.patientRole[0].patient;
			$scope.contactInfo = {};
			$scope.contactInfo.telecom = ccd.recordTarget.patientRole[0].telecom;
			$scope.contactInfo.address = ccd.recordTarget.patientRole[0].addr;
			$scope.components = ccd.component.structuredBody.component;
			$scope.birthTime = $scope.parseStringToDate($scope.patient.birthTime.value);
			$scope.age = $scope.calcAge();
			$scope.setUpCharts();
		} else if ($scope.currentUser.role === 'patient' && $scope.user) {
			$http.get('ccd/' + $scope.user.Id)
